...
function findYtDlpPath(){
  try {
    if (fs.existsSync(YTDLP_EXE)) return YTDLP_EXE;
    if (process.platform === 'win32') {
  ...
  if (req.method === 'POST' && url.pathname === '/api/download') {
    ...
    req.on('end', async () => {
      try {
        ...
        const template = path.join(DOWNLOAD_DIR, '%(title).80s-%(id)s.%(ext)s');
        const args = [
